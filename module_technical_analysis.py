...
class TechnicalAnalysis:
    ...
    def SMA60(self):
        ...
        self.df['below_SMA60'] = np.where(
            self.df['Close'] < self.df['SMA60'], True, False)
    ...
    def buy_consolidation(self):
        ...
        bullish_engulf = (self.df['Close'] * (1 + self.consolidation_tolerance) >= np.maximum(yesterday_df['Open'], yesterday_df['Close'])) & \
            (self.df['Open'] * (1 - self.consolidation_tolerance) <=
             np.minimum(yesterday_df['Open'], yesterday_df['Close']))

        # Flag
        flag = (yesterday_df['High'] * (1 + self.consolidation_tolerance) >= np.maximum(self.df['Open'], self.df['Close'])) & \
            (yesterday_df['Low'] * (1 - self.consolidation_tolerance)
             <= np.minimum(self.df['Open'], self.df['Close']))

        self.df['buy_consolidation'] = bullish_engulf | (
            flag & today_bullish_candle)
    ...
    def good_to_buy(self, consolidation_days=3):
        ...
        for boll_buy_date in boll_buy_dates:
            ...
            consolidation_dates = self.df.loc[start_date:end_date,
                                              'buy_consolidation'].index
